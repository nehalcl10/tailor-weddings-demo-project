...
This skill makes all the **code-level** changes (env.ts schemas, `.env.example` files, CLAUDE.md docs, and Terraform module wiring). Once the code changes are in, point the user at [`docs/setup/UPDATE_ENV_VARS.md`](../../../docs/setup/UPDATE_ENV_VARS.md) — they populate `.envrc` and run `terraform apply` themselves to roll the variable to staging/production.
...
| `CLAUDE.md` | Add the var to the **Environment Variables → Server** list (required or optional section) |
...
| `infra/terraform/envs/staging/.envrc.example` | Add `export TF_VAR_<name>=""` |
| `infra/terraform/envs/production/.envrc.example` | Same |
| `CLAUDE.md` | Add the var to the **Environment Variables → Web** list |
