...
docker compose ps                                                 # Infra services running? (doc Step 3)
...
**Port 5433 in use:** `lsof -i :5433`. The dev DB publishes to host port 5433 to dodge a host PostgreSQL on 5432. If 5433 is also taken on this machine, edit the `ports:` mapping in `docker-compose.yml` **and** update `DATABASE_URL` in `apps/server/.env` to match. Same pattern for `TEST_DATABASE_URL` and port 5434 in `docker-compose.test.yml` if the test DB later collides.

**MinIO bucket missing:** If `pnpm run setup` finished but the `file-uploads` bucket isn't there, the `minio-init` container errored. Re-run `docker compose run --rm minio-init` from the repo root — it's gated behind the `init` profile so a plain `pnpm infra:up` won't re-trigger it.

**`pnpm run setup` is idempotent** — re-running is always safe. Day-to-day after first setup, only `pnpm infra:up` (start containers) and `pnpm dev` (start app) are needed.
